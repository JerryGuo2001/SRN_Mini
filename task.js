...
function generateUniquePairs() {
    ...
    // Step 1: Group graph indices by node_count
    const groupedByNodeCount = {};
    for (let i = 0; i < graphMetadata.length; i++) {
        const { node_count } = graphMetadata[i];
        if (!groupedByNodeCount[node_count]) {
            groupedByNodeCount[node_count] = [];
        }
        groupedByNodeCount[node_count].push(i);
    }

    // Step 2: For each group, generate unique unordered pairs from different blocks
    for (const nodeCount in groupedByNodeCount) {
        const indices = groupedByNodeCount[nodeCount];
        const pairsInGroup = [];

        // Filter out pairs with the same block_id
        for (let i = 0; i < indices.length; i++) {
            for (let j = i + 1; j < indices.length; j++) {
                const idxA = indices[i];
                const idxB = indices[j];
                if (graphMetadata[idxA].block_id !== graphMetadata[idxB].block_id) {
                    pairsInGroup.push([idxA, idxB]);
                }
            }
        }

        // Shuffle within group
        for (let i = pairsInGroup.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [pairsInGroup[i], pairsInGroup[j]] = [pairsInGroup[j], pairsInGroup[i]];
        }

        // Store shuffled valid pairs and their metadata
        for (const [i, j] of pairsInGroup) {
            pairs.push([i, j]);
            pairMetadata.push({
                indexA: i,
                indexB: j,
    ...
    // Final shuffle of the entire sequence (preserve pairings)
    const combined = pairs.map((pair, idx) => ({ pair, meta: pairMetadata[idx] }));
    for (let i = combined.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [combined[i], combined[j]] = [combined[j], combined[i]];
    }
    pairs = combined.map(d => d.pair);
    pairMetadata = combined.map(d => d.meta);
}
