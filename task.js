...
function generateUniquePairs() {
  ...
  // 1) Group indices by block_id
  const byBlock = new Map();
  for (let i = 0; i < graphMetadata.length; i++) {
    const b = graphMetadata[i].block_id;
    if (!byBlock.has(b)) byBlock.set(b, []);
    byBlock.get(b).push(i);
  }

  // 2) Build all pairs *within* each block
  const allPairs = [];
  for (const [blockId, idxs] of byBlock.entries()) {
    // Deterministic shuffle of indices in each block (optional; keeps order unbiased)
    shuffleInPlaceDeterministic(idxs, randPairs);

    for (let a = 0; a < idxs.length; a++) {
      for (let b = a + 1; b < idxs.length; b++) {
        allPairs.push([idxs[a], idxs[b]]);
      }
    }
  }

  // 3) Deterministically shuffle all within-block pairs to get trial order
  ...
  for (const [i, j] of allPairs) {
    ...
    pairMetadata.push({
      ...
      blockB: graphMetadata[j].block_id,  // will match blockA now
